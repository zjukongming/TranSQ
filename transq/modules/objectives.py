...
def compute_mimic(pl_module, batch):
    ...
    if phase!="train":
        ...
        for sent_ret, sent_tar, s in zip(ret_result[0],ret_result[1],ret_result[2]):
            ...
            trans = [trans_list]
            ref = [[tar_list]]
            bleu_1 = getattr(pl_module, f"{phase}_mimic_bleu1")(ref, trans).cpu().numpy()   
            bleu_2 = getattr(pl_module, f"{phase}_mimic_bleu2")(ref, trans).cpu().numpy()   
            bleu_3 = getattr(pl_module, f"{phase}_mimic_bleu3")(ref, trans).cpu().numpy()   
            bleu_4 = getattr(pl_module, f"{phase}_mimic_bleu4")(ref, trans).cpu().numpy()

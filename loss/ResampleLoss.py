...
class ResampleLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.class_freq = torch.from_numpy(np.asarray(
            mmcv.load(freq_file)['class_freq'])).float().cuda()+eps
    ...
    def forward(self,
        ...
        weight = self.reweight_functions(label)     #确定每个case的权重（若类别平衡则权重平衡；若类别不平衡则强调稀有类）
    ...
    def logit_reg_functions(self, labels, logits, weight=None):
        ...
        if 'init_bias' in self.logit_reg:
            logits += self.init_bias                                                #预测概率同加一个值
        if 'neg_scale' in self.logit_reg:
            logits = logits * (1 - labels) * self.neg_scale  + logits * labels      #把negative_cases的预测结果乘上一个权重neg_scale（若neg_scale>1，则扩大了negative的预测值）
            weight = weight / self.neg_scale * (1 - labels) + weight * labels       #若neg_scale>1，则是缩小neg_cases的weights
        return logits, weight
    ...
    def rebalance_weight(self, gt_labels):
        ...
        weight = torch.sigmoid(self.map_beta * (pos_weight - self.map_gamma)) + self.map_alpha  #越稀有权重越高
        return weight
    ...
    def RW_weight(self, gt_labels, by_class=True):
        ...
        return weight
"""
x = torch.rand(10, 80) - 0.5        #batch_size = 10, class_num = 80
y = (x>0).long()
print(x, y)
RLoss = ResampleLoss(use_sigmoid=True,reweight_func='rebalance',
...
print("MSE_Loss",MSELoss(x.cuda(), y.cuda()).shape)
loss = RLoss(x.cuda(),y.cuda())
print(loss.shape)
"""